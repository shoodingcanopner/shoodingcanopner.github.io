...
비평형 마르코프 연쇄(kinetic scheme)에서 어떤 관측량의 감도(Hill coefficient)가 **perturbation의 support 크기 m**에 의해 bound된다는 것을 증명한 논문. 평형계에서 Hill coefficient ≤ 결합 자리 수 n이라는 사실을 비평형계로 일반화한다. 또한 이 bound를 포화시키는 새로운 비평형 메커니즘인 **nested hysteresis**를 제안한다.
...
- **Support bound**: 임의의 kinetic scheme에서, perturbation x의 support 크기 m이 로그 감도를 bound한다: |d log⟨A⟩/d log x| ≤ m
