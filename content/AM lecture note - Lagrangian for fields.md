...
앞서 직접 유도한 결론 — $\mathcal{L} = -\frac{1}{2}q\ddot{q}$와 $\mathcal{L} = \frac{1}{2}\dot{q}^2$이 total derivative 하나 차이라는 것 — 이 사실 중요한 이유가 여기 있다. 물리적으로 동등하지만, 후자가 경계조건도 단순하고 안정적이다.


## $q$와 $\dot{q}$를 독립변수로 두는 것이 말이 되는가?

라그랑지안 역학을 공부하면서 자연스럽게 드는 의문이다. world line이 시공간에서 연속적으로 그려지려면 미래의 위치가 과거의 속도에 의해 결정될 수밖에 없는데, 그렇다면 $q$와 $\dot{q}$를 독립적으로 취급하는 게 정당한가?

이 혼란은 **두 가지 다른 맥락을 섞는 데서 온다.**

**방정식을 도출하는 단계**에서는 $q$와 $\dot{q}$를 독립변수로 본다. 이건 $\mathcal{L}(q, \dot{q})$라는 함수의 편미분을 정의하기 위한 수학적 장치다. $f(x, y) = x^2 + y^2$에서 $\partial f/\partial x$를 구할 때 $y$를 상수 취급하는 것처럼, 함수의 인자들을 형식적으로 독립적으로 다루는 것이다. 만약 처음부터 $\dot{q} = dq/dt$를 넣어버리면 $\partial\mathcal{L}/\partial\dot{q}$라는 표현 자체가 정의되지 않는다.

**운동방정식이 결정된 이후**에는 $q$와 $\dot{q}$ 사이의 관계가 복원된다. $\dot{q} = dq/dt$라는 구속이 되살아나고, world line은 연속적으로 그려진다. 실제로 오일러-라그랑주 방정식은 2계 ODE이고, 초기조건 $q(t_0)$와 $\dot{q}(t_0)$를 주면 이후 궤적이 완전히 결정된다.

즉 순서는 이렇다:

> $q$와 $\dot{q}$를 독립으로 취급 → 오일러-라그랑주 방정식 도출 → 그 방정식의 해로서 실제 경로를 구할 때 비로소 $\dot{q} = dq/dt$ 관계가 적용됨

비유하자면, 열역학에서 $P$, $V$, $T$를 이상기체 법칙 $PV = nRT$ 없이 독립변수로 취급해서 편미분을 정의하는 것과 같다. 구속 관계는 나중에 적용하는 것이다.
